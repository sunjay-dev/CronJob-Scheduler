...
describe("Log Fetching APIs", () => {
  ...
  describe("GET /api/v1/logs", () => {
    it("should fetch all user logs with pagination", async () => {
      ...
      const response = await request(app).get("/api/v1/logs?page=1&limit=10").set("Authorization", "Bearer token");
  ...
  describe("GET /api/v1/logs/:jobId", () => {
    ...
    it("should fetch logs for a specific job", async () => {
      ...
      const response = await request(app).get(`/api/v1/logs/${validJobId}`).set("Authorization", "Bearer token");
  ...
  describe("GET /api/v1/logs/id/:logId", () => {
    ...
    it("should return 404 if log not found", async () => {
      ...
      const response = await request(app).get(`/api/v1/logs/id/${validLogId}`).set("Authorization", "Bearer token");
