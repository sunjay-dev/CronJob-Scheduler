...
describe("User Schemas", () => {
  ...
  describe("verifyUserSchema", () => {
    it("should validate correct OTP and userId", () => {
      const result = verifyUserSchema.safeParse({ otp: "123456", userId: validId });
      expect(result.success).toBe(true);
    });

    it("should reject invalid OTP", () => {
      const result = verifyUserSchema.safeParse({ otp: "12345", userId: validId });
      expect(result.success).toBe(false);
    });
  });
  ...
  describe("changeUserDetailsSchema", () => {
    ...
    it("should reject invalid mode", () => {
      const result = changeUserDetailsSchema.safeParse({ mode: "invalid-mode" });
      expect(result.success).toBe(false);
    });
  });
});
